...
# Función para fallar, mostrar mensaje y salir del script
_fail() {
    local msg=${1}
...
# check root
[[ $EUID -ne 0 ]] && _fail "Error fatal: Ejecute este script con privilegios de root."

if _command_exists curl; then
    curl_bin=$(which curl)
else
    _fail "ERROR: El comando 'curl' no se encontró."
fi

# Verificar el sistema operativo y establecer la variable release
if [[ -f /etc/os-release ]]; then
    ...
    _fail "No se pudo verificar el sistema operativo, ¡comuníquese con el autor!"
fi
...
arch() {
    ...
    s390x) echo 's390x' ;;
    *) echo -e "${green}¡Arquitectura de CPU no compatible! ${plain}" && rm -f "${cur_dir}/${script_name}" >/dev/null 2>&1 && exit 2;;
    esac
}
...
setup_ip_certificate() {
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}Es configurable manualmente en los ajustes del panel.${plain}"
        echo -e "${yellow}Ruta del certificado: ${certDir}/fullchain.pem${plain}"
        echo -e "${yellow}Ruta de la key privada: ${certDir}/privkey.pem${plain}"
    else
    ...
    echo -e "${yellow}El panel se reiniciará automáticamente en cada renovación.${plain}"
    return 0
}
...
prompt_and_setup_ssl() {
    ...
    2)
        ...
        # Reiniciar el panel después de configurar SSL (el reinicio aplica la nueva configuración del certificado)
        if [[ $release == "alpine" ]]; then
...
config_after_update() {
    echo -e "${yellow}Configuración de x-ui:${plain}"
    ${xui_folder}/x-ui setting -show true
    ${xui_folder}/x-ui migrate

    # Detectar correctamente si el certificado está vacío verificando si la línea cert: existe y tiene contenido después
    ...
    # Manejar webBasePath faltante o demasiado corto
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        echo -e "${yellow}WebBasePath no existe o es demasiado corto. Generando uno nuevo...${plain}"
    ...
    # Verificar y solicitar configuración de SSL si falta
    if [[ -z "$existing_cert" ]]; then           
        echo ""
        echo -e "${green}════════════════════════════════════════════${plain}"
        echo -e "${green}     ⚠ NO SE DETECTÓ CERTIFICADO SSL ⚠     ${plain}"
        echo -e "${green}════════════════════════════════════════════${plain}"
        ...
        if [[ -z "${server_ip}" ]]; then
            echo -e "${red}No se pudo detectar la IP del servidor${plain}"
            echo -e "${yellow}Por favor, configure SSL manualmente usando: x-ui${plain}"
            return
        fi

        # Solicitar y configurar SSL (dominio o IP)
        prompt_and_setup_ssl "${config_port}" "${config_webBasePath}" "${server_ip}"
            
            # Mostrar información de acceso
            echo ""
            echo -e "${green}════════════════════════════════════════════${plain}"
            echo -e "${green}       Información de acceso al panel       ${plain}"
            echo -e "${green}════════════════════════════════════════════${plain}"
        ...
        echo -e "${green}El certificado SSL ya está configurado${plain}"
        # Mostrar la URL de acceso con el certificado existente
        local cert_domain=$(basename "$(dirname "$existing_cert")")
        echo ""
        echo -e "${green}═══════════════════════════════════════════${plain}"
        echo -e "${green}     Información de acceso al panel        ${plain}"
        echo -e "${green}═══════════════════════════════════════════${plain}"
        echo -e "${green}URL de acceso: https://${cert_domain}:${existing_port}/${existing_webBasePath}${plain}"           
        echo -e "${green}═══════════════════════════════════════════${plain}"
    fi
}
...
update_x-ui() {
    ...
    if [ -f "${xui_folder}/x-ui" ]; then
        ...
        echo -e "${green}Versión actual de x-ui: ${current_xui_version}${plain}"
    else
        _fail "ERROR: Versión actual de x-ui: desconocido"
    fi
    
    echo -e "${green}Descargando nueva versión de x-ui...${plain}"
    
    last_version=$(${curl_bin} -Ls "https://api.github.com/repos/emirjorge/3x-ui_es/releases/latest" 2>/dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    if [[ ! -n "$last_version" ]]; then
        echo -e "${yellow}Intentando obtener la versión usando IPv4...${plain}"
        last_version=$(${curl_bin} -4 -Ls "https://api.github.com/repos/emirjorge/3x-ui_es/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$last_version" ]]; then
            _fail "ERROR: No se pudo obtener la versión de x-ui. Puede deberse a restricciones de la API de GitHub, inténtalo más tarde"
        fi
    fi
    echo -e "Se obtuvo la última versión de x-ui: ${last_version}, comenzando la instalación..."
    ${curl_bin} -fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/emirjorge/3x-ui_es/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz 2>/dev/null
    if [[ $? -ne 0 ]]; then
        echo -e "${yellow}Intentando descargar usando IPv4...${plain}"
        ${curl_bin} -4fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/emirjorge/3x-ui_es/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz 2>/dev/null
        if [[ $? -ne 0 ]]; then
            _fail "ERROR: No se pudo descargar x-ui. Asegúrate de que tu servidor pueda acceder a GitHub"
        fi
    fi
    
    if [[ -e ${xui_folder}/ ]]; then
        echo -e "${green}Deteniendo x-ui...${plain}"
        if [[ $release == "alpine" ]]; then
            if [ -f "/etc/init.d/x-ui" ]; then
                rc-service x-ui stop >/dev/null 2>&1
                rc-update del x-ui >/dev/null 2>&1
                echo -e "${green}Eliminando la versión antigua del servicio de x-ui...${plain}"
                rm -f /etc/init.d/x-ui >/dev/null 2>&1
            else
                rm x-ui-linux-$(arch).tar.gz -f >/dev/null 2>&1
                _fail "ERROR: El servicio de x-ui no está instalado."
            fi
        else
            if [ -f "${xui_service}/x-ui.service" ]; then
                systemctl stop x-ui >/dev/null 2>&1
                systemctl disable x-ui >/dev/null 2>&1
                echo -e "${green}Eliminando la versión antigua de la unidad systemd...${plain}"
                rm ${xui_service}/x-ui.service -f >/dev/null 2>&1
                systemctl daemon-reload >/dev/null 2>&1
            else
                rm x-ui-linux-$(arch).tar.gz -f >/dev/null 2>&1
                _fail "ERROR: la unidad systemd de x-ui no está instalada."
            fi
        fi
        echo -e "${green}Eliminando la versión antigua de x-ui...${plain}"
        ...
        echo -e "${green}Eliminando la versión antigua de xray...${plain}"
        rm ${xui_folder}/bin/xray-linux-amd64 -f >/dev/null 2>&1
        echo -e "${green}Eliminando los archivos antiguos README y LICENSE...${plain}"
        rm ${xui_folder}/bin/README.md -f >/dev/null 2>&1
        rm ${xui_folder}/bin/LICENSE -f >/dev/null 2>&1
    else
        rm x-ui-linux-$(arch).tar.gz -f >/dev/null 2>&1
        _fail "ERROR: x-ui no está instalado."
    fi
    
    echo -e "${green}Instalando la nueva versión de x-ui...${plain}"
    tar zxvf x-ui-linux-$(arch).tar.gz >/dev/null 2>&1
    rm x-ui-linux-$(arch).tar.gz -f >/dev/null 2>&1
    cd x-ui >/dev/null 2>&1
    chmod +x x-ui >/dev/null 2>&1
    
    # Verificar la arquitectura del sistema y renombrar el archivo según corresponda
    ...
    echo -e "${green}Descargando e instalando el script x-ui.sh...${plain}"
    ${curl_bin} -fLRo /usr/bin/x-ui https://raw.githubusercontent.com/emirjorge/3x-ui_es/main/x-ui.sh >/dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        echo -e "${yellow}Intentando descargar x-ui usando IPv4...${plain}"
        ${curl_bin} -4fLRo /usr/bin/x-ui https://raw.githubusercontent.com/emirjorge/3x-ui_es/main/x-ui.sh >/dev/null 2>&1
        if [[ $? -ne 0 ]]; then
            _fail "ERROR: No se pudo descargar el script x-ui.sh, asegúrate de que tu servidor pueda acceder a GitHub"
        fi
    fi
    ...
    echo -e "${green}Cambiando propietario...${plain}"
    chown -R root:root ${xui_folder} >/dev/null 2>&1
    
    if [ -f "${xui_folder}/bin/config.json" ]; then
        echo -e "${green}Cambiando permisos del archivo de configuración...${plain}"
        chmod 640 ${xui_folder}/bin/config.json >/dev/null 2>&1
    fi
    
    if [[ $release == "alpine" ]]; then
        echo -e "${green}Descargando e instalando la unidad de inicio x-ui.rc...${plain}"
        ${curl_bin} -fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/emirjorge/3x-ui_es/main/x-ui.rc >/dev/null 2>&1
        if [[ $? -ne 0 ]]; then
            ${curl_bin} -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/emirjorge/3x-ui_es/main/x-ui.rc >/dev/null 2>&1
            if [[ $? -ne 0 ]]; then
                _fail "ERROR: No se pudo descargar la unidad de inicio x-ui.rc, asegúrese de que su servidor pueda acceder a GitHub"
            fi
        fi
        ...
        if [ -f "x-ui.service" ]; then
            echo -e "${green}Instalando la unidad systemd...${plain}"
            cp -f x-ui.service ${xui_service}/ >/dev/null 2>&1
            if [[ $? -ne 0 ]]; then
                echo -e "${red}Error al copiar x-ui.service${plain}"
                exit 1
            fi
        else
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    if [ -f "x-ui.service.debian" ]; then
                        echo -e "${green}Instalando la unidad systemd tipo Debian...${plain}"
                ...
                arch | manjaro | parch)
                    if [ -f "x-ui.service.arch" ]; then
                        echo -e "${green}Instalando la unidad systemd tipo Arch...${plain}"
                ...
                *)
                    if [ -f "x-ui.service.rhel" ]; then
                        echo -e "${green}Instalando la unidad systemd tipo RHEL...${plain}"
            ...
            if [ "$service_installed" = false ]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/emirjorge/3x-ui_es/main/x-ui.service.debian >/dev/null 2>&1
                    ;;
                    arch | manjaro | parch)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/emirjorge/3x-ui_es/main/x-ui.service.arch >/dev/null 2>&1
                    ;;
                    *)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/emirjorge/3x-ui_es/main/x-ui.service.rhel >/dev/null 2>&1
                    ;;
                esac
