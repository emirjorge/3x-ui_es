...
uninstall() {
    ...
    echo ""
    echo -e "Se desinstaló correctamente. Si desea eliminar este script, después de salir del script, ejecute ${green}rm /usr/bin/x-ui -f${plain} para eliminarlo."
    echo ""
...
open_ports() {
    ...
    read -p "Ingrese los puertos que desea abrir (ejem. 80,443,2053 o rango 400-500): " ports

    # Check if the input is valid
    if ! [[ $ports =~ ^([0-9]+|[0-9]+-[0-9]+)(,([0-9]+|[0-9]+-[0-9]+))*$ ]]; then
        echo "Error: entrada no válida. Introduzca una lista de puertos separados por comas o un rango de puertos (e.g. 80,443,2053 o 400-500)." >&2
        exit 1
    fi
...
update_geo() {
    ...
    systemctl start x-ui
    echo -e "${green}Geosite.dat + Geoip.dat + Iran.dat se han actualizado correctamente en la carpeta bin '${binfolder}'!${plain}"
    before_show_menu
}
...
iplimit_main() {
    ...
    case "$choice" in
        ...
        2)
            read -rp "Ingrese la nueva duración del Ban en minutos [predeterminado 5]: " NUM
            if [[ $NUM =~ ^[0-9]+$ ]]; then
